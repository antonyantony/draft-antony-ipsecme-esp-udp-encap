...
for this mechanism by including SA_RESOURCE_INFO ([[RFC9611]], see
[[Capability Announcement]]) in the IKE_AUTH exchange.
...
peers have negotiated per-resource Child SAs using SA_RESOURCE_INFO
per [[RFC9611]], and the mechanism defined in this document is in
use, ESP-in-UDP packets belonging to a per-resource Child SA MAY use
...
Support for per-resource Child SA creation is signaled by including
SA_RESOURCE_INFO ([[RFC9611]]) in the IKE_AUTH exchange.  Both peers
MUST include this notification to enable the mechanism.  If either
peer omits SA_RESOURCE_INFO from IKE_AUTH, per-resource Child SAs
MUST NOT be created; both peers MUST use the Fallback SA for all
traffic.

The SA_RESOURCE_INFO notification MAY carry an optional Resource
Identifier in its notification data.  When present, this field is
opaque and intended for debugging purposes only, as defined in
[[RFC9611]].
...
new UDP source port, and having exchanged SA_RESOURCE_INFO in
IKE_AUTH, the responder MUST:
